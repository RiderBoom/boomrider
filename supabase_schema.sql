...
-- ── Guard _wallet_credit against empty/null p_user_id ──────────────────────────
...
-- ── Internal function for financial settlement ─────────────────────────────
...
BEGIN
  ...
  ELSIF v_method = 'cash' THEN
    IF v_type IN ('parcel', 'ride', 'service') THEN
      ...
      END IF;
    ELSE -- food
      PERFORM public._wallet_credit(v_rider_uid, -v_food,      p_order_id, 'หักยอดร้าน(สด)');
      PERFORM public._wallet_credit(v_merch_uid,  v_merch_inc, p_order_id, 'รายได้ร้านค้า(สด)');
      PERFORM public._wallet_credit(v_admin_uid,  v_gp,        p_order_id, 'GP(สด)');
    END IF;
  END IF;
...
-- ── Public authorization wrapper for process_order_settlement ───────────────
