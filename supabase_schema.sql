...
-- ── Rider Order Acceptance & Cash Liability Eligibility RPCs ─────────────────
...
BEGIN
  ...
  -- Fetch rider row
  SELECT * INTO v_rider FROM riders WHERE id = p_rider_id OR user_id = p_rider_id OR data->>'userId' = p_rider_id LIMIT 1;
  ...
  IF v_req_liability > 0 THEN
    v_active_liability := public.get_rider_active_cash_liability(v_rider.id, p_rider_user_id);
  ...
  UPDATE riders SET is_available = false WHERE id = v_rider.id;
...
DECLARE
  ...
  v_matched_rider_id TEXT;
BEGIN
  IF auth.uid() IS NULL THEN
    RAISE EXCEPTION 'authentication_required' USING ERRCODE = '42501';
  END IF;

  SELECT id, COALESCE(user_id, data->>'userId') INTO v_matched_rider_id, v_rider_user_id
  FROM public.riders
  WHERE id = p_rider_id OR user_id = p_rider_id OR data->>'userId' = p_rider_id
  LIMIT 1;

  IF v_rider_user_id IS NULL OR (v_rider_user_id <> auth.uid()::text AND NOT public.is_admin(auth.uid())) THEN
    SELECT id, COALESCE(user_id, data->>'userId') INTO v_matched_rider_id, v_rider_user_id
    FROM public.riders
    WHERE user_id = auth.uid()::text OR data->>'userId' = auth.uid()::text
    LIMIT 1;
  END IF;
  ...
  RETURN public.accept_order_direct_internal(p_order_id, COALESCE(v_matched_rider_id, p_rider_id), v_rider_user_id);
END;
$$;
