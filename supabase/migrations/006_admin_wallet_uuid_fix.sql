-- ══════════════════════════════════════════════════════════════════════════════
-- BoomRider — Fix process_order_settlement: resolve admin email → UUID
-- Run in Supabase SQL Editor (Dashboard → SQL Editor → New query)
-- ══════════════════════════════════════════════════════════════════════════════
--
-- Problem: v_admin_uid was hardcoded as email 'boomzalnw2@gmail.com'
--   but wallets table uses auth UUID as user_id.
--   _wallet_credit(email, ...) finds no wallet row → GP never written to DB.
--
-- Fix: resolve admin email → UUID via profiles table at runtime.
-- ══════════════════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_rider_uid  TEXT;
  v_merch_uid  TEXT;
  v_admin_uid  TEXT;  -- resolved from profiles at runtime
BEGIN
  -- Resolve admin UUID from profiles (wallet is keyed by auth UUID, not email)
