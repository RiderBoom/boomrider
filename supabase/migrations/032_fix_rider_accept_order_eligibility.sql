-- ══════════════════════════════════════════════════════════════════════════════
-- 032_fix_rider_accept_order_eligibility.sql
-- 1. Enhances rider user ID resolution in accept_order_direct & internal RPCs
-- 2. Uses COALESCE(user_id, data->>'userId') to handle both column & JSONB schemas
-- 3. Provides fallback authorization matching auth.uid() against riders table
-- ══════════════════════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Fetch rider row (check id, user_id column, or data->>'userId')
...
BEGIN
  ...
  IF v_rider_user_id IS NULL OR (v_rider_user_id <> auth.uid()::text AND NOT public.is_admin(auth.uid())) THEN
    -- Fallback: lookup rider row matching current authenticated user
    SELECT id, COALESCE(user_id, data->>'userId') INTO v_matched_rider_id, v_rider_user_id
    FROM public.riders
